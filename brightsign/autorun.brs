...
Sub Main()
    ...
    ' Build safe strings before launching (concatenating Invalid crashes &hf4)
    q = Chr(34)
    safeScreenId = ""
    safeToken = ""
    If cfg <> Invalid Then
    ...
    ' Launch widget with the CORRECT page from the start.
    ' First boot -> activate.html; normal boot -> index.html.
    ' Do NOT create with activate.html then SetUrl to index.html — the
    ' load-complete event timing becomes racy and config injection is missed.
    startUrl = "file:///index.html"
    If isFirstBoot Then startUrl = "file:///activate.html"
    ...
    h.SetPort(msgPort)
    h.Show()
    Print "[SF] Widget shown with url=" + startUrl
    ...
    ' Normal boot — index.html is already loading.
    ...
    ' exact load-complete moment, one of these repeated injections will land.
    Print "[SF] Injecting config repeatedly to guarantee delivery..."
    injTimer = NewTimer()
    idxReady = false
    While GetElapsedSecs(injTimer) < 12
        idxMsg = Wait(1000, msgPort)
        ...
        If Type(idxMsg) = "roHtmlWidgetEvent" Then
            ...
            If Type(idxData) = "roAssociativeArray" Then
                If idxData.reason = "load-complete" Then
                    Print "[SF] index.html load-complete — config injected"
                    h.InjectJavascript(cfgInject)
                    idxReady = true
                    Exit While
                End If
        ...
        End If
    End While
    If Not idxReady Then Print "[SF] No load-complete seen, config injected repeatedly"
