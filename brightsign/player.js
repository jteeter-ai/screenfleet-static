...
// Called by autorun.brs via InjectJavascript to provide config (BrightSign can't fetch file://)
window.sfSetConfig = function(apiOrigin, screenId, screenToken, screenWidth, screenHeight) {
  window.__SF_CONFIG__ = {
    apiOrigin: apiOrigin,
    screenId: screenId,
  ...
  console.warn('[SF] sfSetConfig called. screenId=' + screenId);
  // If boot already ran and failed, restart it
  if (!CFG && typeof boot === 'function') { boot(); }
};
...
async function boot() {
  ...
  // BrightSign HTML widget cannot fetch() file:// URLs.
  // autorun.brs injects config via window.__SF_CONFIG__ before this runs.
  // Fall back to fetch() for hosted (http/https) environments.
  if (window.__SF_CONFIG__) {
    CFG = window.__SF_CONFIG__;
    console.warn('[SF] Config from injection. screenId=' + CFG.screenId);
  } else {
    try {
      var r = await fetch('./config.json');
      CFG = await r.json();
      console.warn('[SF] Config from fetch. screenId=' + CFG.screenId);
    } catch(e) {
      // Wait for autorun.brs to inject config (retry a few times)
      console.warn('[SF] config.json fetch failed, waiting for injection...');
      var tries = 0;
      while (!window.__SF_CONFIG__ && tries < 30) {
        await new Promise(function(res){ setTimeout(res, 500); });
        tries++;
      }
      if (window.__SF_CONFIG__) {
        CFG = window.__SF_CONFIG__;
        console.warn('[SF] Config injected after wait. screenId=' + CFG.screenId);
      } else {
        showError('No config available (injection timeout)');
        return;
      }
    }
  }
